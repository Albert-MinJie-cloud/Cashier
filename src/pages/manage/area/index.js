...
function Area() {
    ...
    const createItem = (data) => {
        common.ajax('post', '/manage/area/create', data).then(() => {
            setReLoad(!reLoad)
    ...
    const updateItem = (data) => {
        common.ajax('post', '/manage/area/update', data).then(() => {
            setReLoad(!reLoad)
        }).finally(_ => {
    ...
    // 启用区域
    const enableItem = areaId => {
        const data = {areaId: areaId}
        common.ajax('post', '/manage/area/enable', data).then(() => {
            setReLoad(!reLoad)
        })
    }
    // 停用区域
    const disableItem = areaId => {
        const data = {areaId: areaId}
        common.ajax('post', '/manage/area/disable', data).then(() => {
            setReLoad(!reLoad)
        })
    }
    // 删除区域
    const deleteItem = areaId => {
        const data = {areaId: areaId}
        common.ajax('post', '/manage/area/delete', data).then(() => {
            setReLoad(!reLoad)
        })
    }
    ...
    return (
        <div>
            <Menu num={13}/>
            <div className={'Area'} style={{marginLeft: 60}}>
                ...
                <Modal title="区域新增" visible={isCreateModelShow} footer={null}
                ...
                <Modal title="区域编辑" visible={isUpdateModelShow} footer={null}
